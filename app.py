...
def dashboard_page():
    ...
    if role == "admin":
        ...
        df_queries = pd.DataFrame(queries, columns=[
            "Query ID", "Client ID", "Email", "Mobile Number",
            "Query Heading", "Query Description", "Status",
            "Created Time", "Closed Time"
        ])

        # Normalize status and strip spaces
        df_queries['Status'] = df_queries['Status'].replace({"Opened": "Open"})
        df_queries['Status'] = df_queries['Status'].str.strip()
        ...
        if not df_queries.empty:
            ...
            df_queries['Created Time'] = pd.to_datetime(df_queries['Created Time'], errors='coerce')
